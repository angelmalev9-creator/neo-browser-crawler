...
async function expandHiddenContent(page) {
  try {
    await page.evaluate(async () => {
      ...
      // ── 1. aria-expanded="false" → клик за отваряне ──────────────────────
      ...
      // ── 4. "Виж детайли" / "Повече" / "Details" бутони ───────────────────
      const textTriggerRe = /виж детайли|виж повече|повече информация|детайли|details|разгъни|покажи|show more|read more|expand|see more/i;
      ...
      // Финално изчакване за CSS анимации
      await sleep(200);
    });
...
async function processPage(page, url, base, stats, siteMaps, capabilitiesMaps) {
  ...
  try {
    ...
    // ── UI-AWARE: кликай accordions, tabs, "Виж детайли" преди извличане ──
